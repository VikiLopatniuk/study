...
# With absolute path
# current_lesson_path = Path("/home/ivan/PycharmProjects/Tutorials/Viki/lessons/12")
# With relative paath
# Take the current path from the file execution
...
Grab the two files from the folder
ideas_for_test/work_with_csv
...
Validate that all files are in the folder
ideas_for_test/work_with_json
are valid json.
output the result for an invalid file through the logger at the error level
"""
...
create a search function by group/number and so on
by the timingExbytes/incoming value
display the search result through the logger at the info level
"""
